...
const CONFIG_SCHEMA = AbstractConfigServiceSchema.keys({
  ...
  fileServiceUrl: Joi.string().uri().required(),
...
export class ConfigService extends AbstractConfigService {
  ...
  httpPort = Number(process.env.APP_PORT || process.env.APP_PORT_FILE) || 8082;

  externalServiceUrl = switchEnv(
    {
      e2e: 'http://localhost:8080',
    },
    String(process.env.APP_SERVICE_URL || process.env.APP_SERVICE_URL_FILE || ''),
  );
  ...
  fileServiceUrl = switchEnv(
    {
      e2e: 'http://file-service/',
    },
    String(process.env.APP_SERVICE_URL_FILE || ''),
  );
    ...
    ...switchEnv(
      {
        ...
        production: {
          json: [0, '0', false, 'false'].includes(process.env.LOG_JSON || '') ? false : true,
          consoleLevel: (process.env.LOG_LEVEL as LogLevels) || LogLevels.Info,
        },
      },
      {
        json: [1, '1', true, 'true'].includes(process.env.LOG_JSON || '') ? true : false,
